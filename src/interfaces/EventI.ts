...
export class DaemonEvent implements EventI {
    ...
    getPrintStr(machine: boolean) {
        return `${TAB}[Daemon Check]<color:magenta> ${this.sentTo} - ${this.errorCode} - ${this.errorMsg}<color:grey>`;
    }
...
export class TransferEvent implements EventI {
    ...
    getPrintStr(machine: boolean) {
        ...
        return `❯ [${participant}]<color:yellow> ${from} →<color:yellow> ${this.reason}<color:grey> →<color:yellow> ${to}`;
    }
...
export class MessageEvent implements EventI {
    ...
    getPrintStr(machine: boolean) {
        ...
        participant = this.fromType === 'Agent' ? participant + '<color:blue>' : participant + '<color:green>';
...
export class StartEvent implements EventI {
    ...
    getPrintStr(machine?: boolean) {
        ...
        return `❯❯❯ Start<color:green>` + ` → <color:yellow>${skill}`;
    }
...
export class EndEvent implements EventI {
    ...
    getPrintStr() {
        return `❮❮❮ End by: ${this.reason}/${this.reasonDescription}<color:red>`;
    }
...
export class ParticipantEvent implements EventI {
    ...
    getPrintStr(machine: boolean) {
        ...
        return `${TAB}❯ ${participant}: ${this.permission}<color:grey>`;
    }
